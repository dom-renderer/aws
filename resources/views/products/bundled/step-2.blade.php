...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        ...
        <div class="card mt-4">
            <div class="card-header">
                <h5 class="mb-0">Final Price</h5>
            </div>
            <div class="card-body">
                <h3 class="mb-0" id="finalPriceDisplay">$0.00</h3>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateFinalPrice() {
        ...
        if (priceMode === 'sum') {
            basePrice = bundleState.reduce((sum, item) => {
                const data = bundleMap[item.key];
                if (!data) return sum;
                const unit = (data.units || []).find(u => u.id === item.selectedUnitId);
                if (!unit) return sum;
            ...
            basePrice = parseFloat(fixedPriceInput.value) || 0;
        }
        ...
        if (discountMode === 'fixed') {
            discountAmount = parseFloat(document.getElementById('discountFixedInput').value) || 0;
        } else {
    ...
    fixedPriceInput.addEventListener('input', updateFinalPrice);
    discountInputs.forEach(input => {
