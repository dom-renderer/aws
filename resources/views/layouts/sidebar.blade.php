<nav id="sidebar" class="sidebar">
    ...
    <div class="sidebar-content">
        ...
        <div class="sidebar-user">
            <img src="{{ auth()->guard('web')->user()->userprofile }}" class="img-fluid rounded-circle mb-2" />
            <div class="fw-bold"> {{ auth()->guard('web')->user()->name }} </div>
            <small> {{ implode(', ', auth()->guard('web')->user()->roles()->pluck('name')->toArray()) }} </small>
        </div>
        ...
        <ul class="sidebar-nav">
            ...
            @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('users.index') || auth()->guard('web')->user()->can('roles.index'))
            ...
            @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('customers.index'))
            <li class="sidebar-item @if( request()->segment(1) == 'customers') active @endif">
                <a href="{{ route('customers.index') }}" class="sidebar-link">
                    <i class="align-middle me-2 fas fa-fw fa-user-tie"></i> <span class="align-middle"> Customer </span>
                </a>
            </li>
            @endif

            @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('suppliers.index'))
            ...
            @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('warehouses.index'))
            ...
            @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('locations.index'))
            <li class="sidebar-item @if( request()->segment(1) == 'locations') active @endif">
                <a href="{{ route('locations.index') }}" class="sidebar-link">
                    <i class="align-middle me-2 fas fa-fw fa-map-marker-alt"></i> <span class="align-middle">Store Locations</span>
                </a>
            </li>
            @endif

            @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('categories.index') || auth()->guard('web')->user()->can('products.index') || auth()->guard('web')->user()->can('brands.index'))
            <li class="sidebar-item @if( request()->segment(1) == 'categories' || request()->segment(1) == 'products' || request()->segment(1) == 'brands') active @endif">
                ...
                <ul id="catalogMenu" class="sidebar-dropdown list-unstyled collapse" data-bs-parent="#sidebar">
                    @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('categories.index'))
                    ...
                    @endif
                    @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('products.index'))
                    ...
                    @endif
                    @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('brands.index'))
                    <li class="sidebar-item @if(request()->segment(1) == 'brands') active @endif"><a class='sidebar-link' href='{{ route('brands.index') }}'>Brands</a></li>
                    @endif
                    @if(auth()->guard('web')->user()->isAdmin() || auth()->guard('web')->user()->can('home-page-settings.index'))
                    <li class="sidebar-item @if(request()->segment(1) == 'home-page-settings') active @endif"><a class='sidebar-link' href='{{ route('home-page-settings.index') }}'>Home Page Settings</a></li>
                    @endif
                </ul>
            </li>
            @endif
