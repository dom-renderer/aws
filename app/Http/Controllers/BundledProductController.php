...
class BundledProductController extends Controller
{
    ...
    public static function view($product, $step, $type)
    {
        ...
        $reviewData = [
            ...
            'bundle' => [
                'items' => ProductBundle::where('product_id', $product->id)
                    ->with(['sourceProduct', 'sourceVariant', 'unit'])
                    ->get()
                    ->map(function ($item) {
                        $productName = $item->sourceProduct->name ?? 'Unknown Product';
                        $variantName = $item->sourceVariant ? $item->sourceVariant->name : null;
                        $unitName = $item->unit->title ?? 'Unit';
                        
                        $unitPrice = self::resolveUnitPriceDirect(
                ...
                'pricing' => [
                    'mode' => $product->bundled_product_price_type == 1 ? 'Fixed Price' : 'Sum of Products',
                    'fixed_price' => $product->bundled_product_fixed_price,
                    'discount_mode' => $product->bundled_product_discount_type == 0 ? 'Percentage' : 'Fixed',
                    'discount_value' => $product->bundled_product_discount,
                ]
            ]
        ];

        $subtotal = $reviewData['bundle']['items']->sum('total_price');
        $basePrice = $product->bundled_product_price_type == 1 ? $product->bundled_product_fixed_price : $subtotal;
        
        $discountAmount = 0;
        if ($product->bundled_product_discount_type == 0) {
            $discountAmount = $basePrice * ($product->bundled_product_discount / 100);
        } else {
            $discountAmount = $product->bundled_product_discount;
        }
        
        $finalPrice = max(0, $basePrice - $discountAmount);
        $reviewData['bundle']['pricing']['final_price'] = $finalPrice;
        $reviewData['bundle']['pricing']['subtotal'] = $subtotal;
    ...
    private static function prepareBundleResources(Product $product): array
    {
        ...
        $tierCache = ProductTierPricing::whereIn('product_id', $allProductIds)
            ...
            ->groupBy(function ($tier) {
                return $tier->product_id . ':' . ($tier->product_variant_id ?: '0') . ':' . $tier->unit_type . ':' . $tier->product_additional_unit_id;
            });
    ...
    private static function buildUnitList(Product $product, ?ProductVariant $variant, $baseCache, $additionalCache, $tierCache): array
    {
        ...
        foreach ($baseCache->get($key, collect()) as $baseUnit) {
            $units[] = [
                ...
                'quantity' => 1,
                'price' => self::resolvePriceFromCache($tierCache, $product->id, $variant?->id, 0, $baseUnit->id, $fallback),
        ...
        foreach ($additionalCache->get($key, collect()) as $additionalUnit) {
            $units[] = [
                ...
                'price' => self::resolvePriceFromCache($tierCache, $product->id, $variant?->id, 1, $additionalUnit->id, $fallback),
    ...
    private static function resolvePriceFromCache($tierCache, int $productId, ?int $variantId, int $unitType, int $unitId, float $fallback): float
    {
        $key = $productId . ':' . ($variantId ?: '0') . ':' . $unitType . ':' . $unitId;
    ...
    private static function calculateBundleSubtotal(array $items): float
    {
        ...
        foreach ($items as $item) {
            $price = self::resolveUnitPriceDirect($item['source_product_id'], $item['unit_id'], $item['unit_type'], $item['source_variant_id']);
            $total += $price * $item['quantity'];
        }
        return round($total, 2);
    }

    private static function resolveUnitPriceDirect(int $productId, int $unitRowId, int $unitType, ?int $variantId = null): float
    {
