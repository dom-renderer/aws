                ...
                if ($request->ajax()) {
                    ...
                    switch ($request->op) {
                                ...
                                foreach ($combinations as $set) {
                                    ...
                                    if ($existingVariant) {
                                        ...
                                        $variant = ProductVariant::create([
                                            ...
                                            'name' => $name,
                                            'sku' => $sku,
                                            'short_url' => strtolower(str_replace($sku, ' ', '-')) . '-' . uniqid(),
                                            'barcode' => null,
                                            'status' => 1,
                                        ]);
                                    ...
                                    $parts = ProductAttribute::whereIn('id', $set)->pluck('value')->toArray();
                                    $primaryImg = ProductVariantImage::where('variant_id', $variant->id)
                                        ->where('is_primary', 1)
                                        ->first();
                                    
                                    $secondaryImgs = ProductVariantImage::where('variant_id', $variant->id)
                                        ->where('is_primary', 0)
                                        ->get()
                                        ->map(function ($img) {
                                            return [
                                                'id' => $img->id,
                                                'url' => asset('storage/' . $img->file),
                                            ];
                                        })
                                        ->toArray();
                                    ...
                                    $items[] = [
                                        ...
                                        'attributes' => $parts,
                                        'image' => $primaryImg ? asset('storage/' . $primaryImg->file) : null,
                                        'primary_image' => $primaryImg ? asset('storage/' . $primaryImg->file) : null,
                        ...
                        case 'upload-secondary-images':
                            ...
                            // Get all secondary images for this variant
                            $allSecondaryImages = ProductVariantImage::where('variant_id', $variant->id)
                                ->where('is_primary', 0)
                                ->get()
                                ->map(function ($img) {
                                    return [
                            ...
                            return response()->json(['images' => $allSecondaryImages]);

                        case 'delete-image':
                            $request->validate([
                                'id' => 'required|integer|exists:product_variant_images,id',
                            ...
                            // Don't allow deleting primary image through this endpoint
                            if ($image->is_primary) {
                                return response()->json(['message' => 'Cannot delete primary image using this endpoint'], 422);
                            }
                        ...
                        case 'list':
                            $variants = ProductVariant::where('product_id', $product->id)
                                ->with(['attributes.attribute', 'product'])
                                ->get();

                            $items = $variants->map(function ($v) {
                                $primaryImg = ProductVariantImage::where('variant_id', $v->id)
                                    ->where('is_primary', 1)
                                    ->first();
                                
                                $secondaryImgs = ProductVariantImage::where('variant_id', $v->id)
                                    ->where('is_primary', 0)
                                    ->get()
                                    ->map(function ($img) {
                                        return [
